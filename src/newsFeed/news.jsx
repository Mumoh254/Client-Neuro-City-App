...
const NewsFeed = () => {
    ...
    const fetchData = async () => {
      try {
        const [newsRes, engagementsRes, analyticsRes] = await Promise.all([
          axios.get('http://localhost:8000/apiV1/smartcity-ke/get-news'),
          axios.get('http://localhost:8000/apiV1/smartcity-ke/engagements'),
  ...
  const handleInteraction = async (type, id, option = null) => {
    try {
      await axios.post(`http://localhost:8000/api/${type}/${id}/react`, {
